...
import (
	...
	"linebotgodmode/library/channel"
	"linebotgodmode/library/hashmap"
	"linebotgodmode/library/modcompact"
...
func CreateNewLogin(token string, num int, mids string, app string, ua string, host string) (*Account, error) {
	...
	HostRE4  := fmt.Sprintf("https://%s.line.naver.jp/RE4", host)
...
func (s *Account) SyncConn() *SyncService.SyncServiceClient {
	...
	return SyncService.NewSyncServiceClientProtocol(connect, protocol, protocol)
}

func (cl *Account) Newsync(count int32) ( ops []*SyncService.Operation, err error ){
	TS := cl.SyncConn()
	res, err := TS.Sync(cl.Ctx, &SyncService.SyncRequest{
...
func GetRoom(to string) *LineRoom {
	...
	new := &LineRoom{Id: to, Userlurk: []string{}, MsgLeave: "See you nix time", WelcomeMsg: "Hallo Welcome Join", MsgLurk: "Hi.", Gowner: []string{}, Gadmin: []string{}, Gban: []string{}, ListInvited: []string{}, Bot: []string{}, GoMid: []string{}, ProKick: false, Limit: false, ProQr: false, ProName: false, ProInvite: false, ProJoin: false, ProCancel: false, AntiTag: false, Automute: false, Lurk: false, Announce: false, Qr: false, Purge: false}
...
func (ve *Account) FindAndAddContactsByMid(mid string) (c string) {
	GLX := []byte{130, 33, 1, 14, 97, 100, 100, 70, 114, 105, 101, 110, 100, 66, 121, 77, 105, 100, 28, 21, 248, 168, 2, 24, 33}
	GLX = append(GLX, GetStringBytes1(mid, false)...)
	GLX = append(GLX, []byte{28, 24, 48, 123, 34, 115, 99, 114, 101, 101, 110, 34, 58, 34, 102, 114, 105, 101, 110, 100, 65, 100, 100, 58, 114, 101, 99, 111, 109, 109, 101, 110, 100, 34, 44, 34, 115, 112, 101, 99, 34, 58, 34, 110, 97, 116, 105, 118, 101, 34, 125, 28, 172, 0, 0, 0, 0, 0}...)
	HTTP, _ := thrift.NewTHttpClient(ve.UrRE4, ve.Transport)
	transport := HTTP.(*thrift.THttpClient)
...
/*func (cl *Account) FindAndAddContactsByMid(mid string) (res map[string]*talkservice.Contact, err error) {
...
func postData(urlx string, formData map[string]string) ( map[string]interface{}, error) {
	...
	// Membuat nilai URL-encoded dari data form
	data := url.Values{}
	for key, value := range formData {
		data.Set(key, value)
	}
	// Membuat request HTTP dengan metode POST dan data form
	...
	// Membuat klien HTTP dan mengirimkan request
	client := &http.Client{}
	resp, err := client.Do(req)
	if err != nil {
		return nil, err
	}
	// Jangan lupa untuk menutup body response setelah digunakan
	defer resp.Body.Close()

	// Baca body dari respon sebagai JSON
	body, err := ioutil.ReadAll(resp.Body)
	if err != nil {
		return nil, fmt.Errorf("Error reading response body: %v", err)
	}

	// Struktur data untuk menyimpan respon JSON
	var responseData map[string]interface{}

	// Unmarshal JSON ke dalam variabel responseData
